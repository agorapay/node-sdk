...
class PaymentMethodApi extends ApiRest_1.default {
    ...
    async listPaymentMethods(options) {
        const result = await this.sendToApiPost("/paymentMethods/list", options);
        return {
    ...
    async getPaymentMethodAliases(options) {
        const result = await this.sendToApiPost("/paymentMethods/getAlias", options);
        return {
    ...
    async removePaymentAlias(options) {
        await this.sendToApiPost("/paymentMethods/removeAlias", options);
    }
}
