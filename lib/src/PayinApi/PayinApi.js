...
const SignedMandateFile_1 = __importDefault(require("../models/SignedMandateFile"));
...
class PayinApi extends apiRest_1.default {
    ...
    payment(options) {
        return new Promise((success, reject) => {
            ...
            return this.sendToApiPost('/payin/payment', Object.assign(options, args)).then((resp) => {
    ...
    /**
     * Submit a payment with payment method, strict mode.
     * @description When your shopper choose a payment method, this call submit the choice and any data if already given.\nThe return can be final, (transaction completed) or ask to authentification details, or redirect the shopper to PSP or 3DS pages.\nMore strict control compared to /payin/payment.
     * @param {PaymentOptionsWithOrderId | PaymentOptionsWithoutOrderId} options Payment options.
     * @prop {PaymentMethod | undefined} transPaymentMethod
     * @prop {number} orderId
     * @prop {string | undefined} orderReference
     ...
     * ````
     */
    paymentSecure(options) {
        return new Promise((success, reject) => {
            const args = Object.assign({});
            if (options.transPaymentMethod) {
                args.transPaymentMethod = { id: options.transPaymentMethod };
            }
            return this.sendToApiPost('/payin/paymentSecure', Object.assign(options, args)).then((resp) => {
                if (+resp.resultCode !== 0)
                    reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
                else {
                    try {
    ...
    paymentDetails(options) {
        return new Promise((success, reject) => {
            return this.sendToApiPost('/payin/paymentDetails', options).then((resp) => {
                if (+resp.resultCode !== 0)
                    reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
                else {
                    try {
    ...
    paymentMethods(options) {
        return new Promise((success, reject) => {
            return this.sendToApiPost('/payin/paymentMethods', options).then((resp) => {
                if (+resp.resultCode !== 0)
                    reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
                else {
                    try {
    ...
    capture(options) {
        return new Promise((success, reject) => {
            return this.sendToApiPost('/payin/capture', options).then((resp) => {
                if (+resp.resultCode !== 0)
                    reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
                if (!resp.orderId === undefined)
                    reject(new Error('Missing required field: orderId'));
                if (!resp.orderStatus ||
                    !Object.values(enums_1.OrderStatus).some((orderStatus) => orderStatus === resp.orderStatus))
                    ...
                    try {
                        success({
                            ...
                            transactionList: resp.transactionList
                                ? resp.transactionList.map((x) => new Transaction_1.default(x))
                                : [],
                            orderId: resp.orderId
                        });
                    }
    ...
    cancel(options) {
        return new Promise((success, reject) => {
            return this.sendToApiPost('/payin/cancel', options).then((resp) => {
                if (+resp.resultCode !== 0)
                    reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
                if (!resp.orderId === undefined)
                    reject(new Error('Missing required field: orderId'));
     ...
     * ````
     */
    adjustPayment(options) {
        return new Promise((success, reject) => {
            return this.sendToApiPost('/payin/adjustPayment', options).then((resp) => {
                if (+resp.resultCode !== 0)
                    reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
                else {
                    try {
     ...
     * ````
     */
    paymentIframe(options) {
        return new Promise((success, reject) => {
            return this.sendToApiPost('/payin/paymentIframe', options).then((resp) => {
                if (+resp.resultCode !== 0)
                    reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
                if (!resp.orderId === undefined)
                    reject(new Error('Missing required field: orderId'));
    ...
    /**
     * Post your payment details and get an authent Code to initiate a payment Frame, strict mode
     * @description "When your shopper is ready to pay, submit your order/payment by this request and get an Authent Code.\nThen save the orderId and open an iframe for the shopper with the authentCode.\nMore strict control compared to /payin/paymentIframe"
     * @param {PaymentIFrameOptions} options
     * @returns {Promise<PaymentIFrameResponse>}
     * @example
     * ````javascript
     *payinApi.paymentIframe({
     *  orderReference: "ref_0102636",
     ...
     * ````
     */
    paymentIframeSecure(options) {
        return new Promise((success, reject) => {
            return this.sendToApiPost('/payin/paymentIframeSecure', options).then((resp) => {
                if (+resp.resultCode !== 0)
                    reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
                if (!resp.orderId === undefined)
                    reject(new Error('Missing required field: orderId'));
    ...
    refund(options) {
        return new Promise((success, reject) => {
            return this.sendToApiPost('/payin/refund', options).then((resp) => {
                if (+resp.resultCode !== 0)
                    reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
                if (!resp.orderId === undefined)
                    reject(new Error('Missing required field: orderId'));
     ...
     * ````
     */
    mandate(transactionId, reference) {
        return new Promise((success, reject) => {
     ...
     * ````
     */
    ticket(transactionId, type, format, message) {
        return new Promise((success, reject) => {
    ...
    reload(options) {
        return new Promise((success, reject) => {
            return this.sendToApiPost('/payin/reload', options).then((resp) => {
                if (+resp.resultCode !== 0)
                    reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
                else {
                    try {
