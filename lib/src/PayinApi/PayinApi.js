                            ...
                            paymentMethodList: resp.paymentMethodList
                                ? resp.paymentMethodList.map((x) => {
                                    const aliasList = x.aliasList
                                        ? x.aliasList.map((y) => new Alias(y.id, y.expirationDate, y.maskedPan, y.label, y.bankCode || y.cardBrand))
                                        : [];
    ...
    capture(options) {
        return new Promise((success, reject) => {
            return this.sendToApiPost('/payin/capture', options).then((resp) => {
                ...
                if (!resp.orderStatus ||
                    !Object.values(OrderStatus).some((orderStatus) => orderStatus === resp.orderStatus))
                    ...
                    try {
                        success({
                            ...
                            transactionList: resp.transactionList
                                ? resp.transactionList.map((x) => new Transaction(x))
                                : [],
                            orderId: resp.orderId
                        });
                    }
