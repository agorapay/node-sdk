...
class PaymentAccountApi extends ApiRest {
            ...
            return this.sendToApiPost('/paymentAccount/list', options).then((resp) => {
                ...
                try {
                    success({
                        ...
                        paymentAccountList: resp.accountList?.map((x) => new PaymentAccount({ account: x })) ?? []
                    });
                }
                catch (err) {
                    reject(err);
    ...
    async setAuthorizedOverdraft(options) {
        return Utils.handleApiResponse(this.sendToApiPost('/paymentAccount/setAuthorizedOverdraft', options), () => null);
    }
}
