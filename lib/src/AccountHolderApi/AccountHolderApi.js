...
import { YesOrNo } from '../../utils/enums.js';
...
class AccountHolderApi extends ApiRest {
        ...
        return new Promise((success, reject) => {
            return this.sendToApiPost('/accountHolder/register', options).then((resp) => {
                if (+resp.resultCode !== 0)
                    ...
                    try {
                        success(new AccountHolder(resp.requirements
                            ? resp.requirements.map((x) => new Requirement(x))
                            : [], resp.physicalPersons
                            ? resp.physicalPersons.map((x) => new PhysicalPersons(x))
     ...
     * ````
     */
    createAccountHolderOnlineRegisterResponse(resp) {
        return new AccountHolderOnlineRegisterResponse(resp.sellerReference, resp.requestId, resp.tokenId, resp.externalReference);
    }
    async onlineRegister(options) {
        return Utils.handleApiResponse(this.sendToApiPost('/accountHolder/onlineRegister', options), (resp) => this.createAccountHolderOnlineRegisterResponse(resp));
    }
    /**
