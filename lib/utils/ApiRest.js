...
class ApiRest {
    ...
    async sendToApiPost(endPoint, payload, isMultiPart = false) {
    ...
    async sendToApiGet(endPoint, payload) {
        return await this.sendAuthenticatedRequest({
            method: "GET",
            url: this.config.baseUrl + endPoint,
            params: payload,
     ...
     */
    async sendAuthenticatedRequest(payload, isMultiPart = false) {
        ...
        this.log(payload);
        try {
            const response = await axios_1.default.request({ ...payload, data: encodedData });
