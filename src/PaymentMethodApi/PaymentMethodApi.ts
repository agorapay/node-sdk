...
class PaymentMethodApi extends ApiRest {
  ...
  removeAlias(options: RemoveAliasOptions): Promise<null> {
    return new Promise((success, reject) => {
      return this.sendToApiPost('/paymentMethod/removeAlias', options).then(
        (resp: any) => {
  ...
  getAlias(options: GetAliasOptions): Promise<PaymentMethodListResponse> {
    return new Promise((success, reject) => {
      return this.sendToApiPost('/paymentMethod/getAlias', options).then(
        (resp: any) => {
          if (+resp.resultCode !== 0)
            reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
          try {
  ...
  listPaymentMethod(
    ...
    return new Promise((success, reject) => {
      return this.sendToApiPost('/paymentMethod/list', options).then(
        (resp: any) => {
          if (+resp.resultCode !== 0)
            reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
          try {
