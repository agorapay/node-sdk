...
class PayinApi extends ApiRest {
   ...
   */
  paymentIframeSecure(
    options: PaymentIFrameOptions
  ): Promise<PaymentIFrameResponse> {
    return new Promise((success, reject) => {
      return this.sendToApiPost('/payin/paymentIframeSecure', options).then(
        (resp: any) => {
          if (+resp.resultCode !== 0)
            reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
          if (!resp.orderId === undefined)
  ...
  reload(options: ReloadOptions): Promise<ReloadResponse> {
    return new Promise((success, reject) => {
      return this.sendToApiPost('/payin/reload', options).then((resp: any) => {
        if (+resp.resultCode !== 0)
          reject(new Error(`${resp.resultCode} - ${resp.resultCodeMessage}`));
        else {
          try {
